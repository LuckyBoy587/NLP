...
- **Dataset Input Rule:** Always read datasets using:
  ...
  ```
- **TensorFlow Warning Policy:** If using TensorFlow related modules or if the `compiler_message` contains `TF-TRT Warning`, include:
  ```python
  import os
...
- **Single Source of Truth:** Use the `test_against_sample_testcases`, `test_against_hidden_testcases`, or `compile_code` MCP tools exclusively.
- **Workflow for Validation:**
  1. **Sample Testcases:** Use `test_against_sample_testcases` to validate the code against public samples.
  2. **Hidden Testcases:** Once sample testcases pass, use `test_against_hidden_testcases` to check against private cases.
  3. **Debugging Loop:** If hidden testcases fail:
     - Use `get_hidden_testcases` to retrieve the failing inputs.
     - Use `compile_code` with the failing input passed to `stdin_input` to reproduce and debug.
     - Fix and repeat until all hidden testcases pass.
- **Iterative Fix Policy:**
  1. Analyze mismatch in `expected`, `actual`, and `testcase_result` fields (from `test_against_sample_testcases`/`test_against_hidden_testcases`) or standard output (from `compile_code`).
  2. **Check for Warnings:** If the `compiler_message` contains `TF-TRT Warning`, apply the **TensorFlow Warning Policy** immediately.
  3. Fix code logically based on testcase failures.
  4. Re-run the appropriate execution tool until all testcases pass or the desired output is achieved.
...
- Never bypass the `test_against_sample_testcases`, `test_against_hidden_testcases`, or `compile_code` MCP tools.
- If the mcp tool call to `test_against_hidden_testcases` returns status as PASS for all testcases, consider that code as the final code.
...
#### `test_against_sample_testcases(code: str) -> dict`
Validates the code against public sample testcases.
- **Input:** `code`: Full source code to validate.

#### `test_against_hidden_testcases(code: str) -> dict`
Validates the code against hidden/private testcases on the platform.
- **Input:** `code`: Full source code to validate.

#### `get_hidden_testcases() -> dict`
Retrieves the input data for hidden testcases of the last validation attempt.
- **Output:** A list of testcase inputs (stdin strings).
